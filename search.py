...
def depthFirstSearch(problem: SearchProblem):
    ...
    getSuccessors(): Returns a list of successor tuples (successor, action, stepCost).
    """
    "*** YOUR CODE HERE ***"
    ...
    while not stack.isEmpty():
        ...
        if curnode not in visited:
            ...
            for nextnode,action,cost in problem.getSuccessors(curnode):
...
def uniformCostSearch(problem):
    ...
    frontier = util.PriorityQueue()  # Use a priority queue for UCS
    explored = set()

    frontier.push((problem.getStartState(), [], 0), 0)  # (state, actions, cost), priority=cost

    while not frontier.isEmpty():
        current_node, actions, cost = frontier.pop()
        ...
        for successor_node, successor_action, successor_cost in successors:
            new_actions = actions + [successor_action]
            new_cost = cost + successor_cost
            frontier.push((successor_node, new_actions, new_cost), new_cost) 
